...
def calMethyLevels(bismark, pos, depth=3):
		...
		if _key in pos:
			for l in pos[_key]:
				if methylation[cols[__TYPE]].has_key(l):
					methylation[cols[__TYPE]][l] += cols[__METHY]
					unmethylation[cols[__TYPE]][l] += cols[__UNMETHY]
				else:
					methylation[cols[__TYPE]][l] = cols[__METHY]
					unmethylation[cols[__TYPE]][l] = cols[__UNMETHY]
	...
	for j in ["CG", "CHG", "CHH"]:
		for i in methylation[j]:
			result[j][i] = methylation[j][i]/float(unmethylation[j][i] + methylation[j][i])
...
if __name__ == '__main__':
	usage = 'python CalculateMethylLevels.py bismarkFile inputBed  depth outfile'
	if len(sys.argv) != 5:
		sys.exit(usage)	
	...
	result = calMethyLevels(sys.argv[1], data, sys.argv[3])
